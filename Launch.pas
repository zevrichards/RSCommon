...
function ConsoleStarterWithOutput(prog, workdir: string; params: array of string; showmode: integer; out output: string; const bSilent: boolean=false): integer;
function StartProgram(prog, workdir: string; params: array of string; out output: string; show: boolean=True; const bSilent: boolean=False ): boolean;
function RunProgramBlocking(prog, workdir: string; params: array of string; out output: string; mode: integer=SW_HIDE): integer;
function RunProgramWaiting(prog, workdir: string; params: array of string; out output: string): integer;
...
function ConsoleStarterWithOutput(prog, workdir: string;
                        params: array of string; showmode: integer; out output: string; const bSilent: boolean=false): integer;
var
...
//some code from https://stackoverflow.com/questions/9119999/getting-output-from-a-shell-dos-app-into-a-delphi-app
...
//  CreatePipe(StdOutPipeRead, StdOutPipeWrite, @SecurAtt, 0);  //for reading any output

  try

//    GetMem(Buffer, READ_BUFFER_SIZE);
    ...
    args := GetEnvironmentVariable('COMSPEC')+' /C '+args+' > outputlog.txt';
    ...
    begin
      ...
      try
        while WaitForSingleObject(ProcInf.hProcess, 50)=WAIT_TIMEOUT do
          Application.ProcessMessages;

        SL := TStringList.Create;
        try
          SL.LoadFromFile(workdir+'outputlog.txt');
          output := SL.text;
        finally
          SL.Free;
        end;
  ...
  try
    ...
    begin
      ...
      try
        ...
        repeat
            ReadFile(StdOutPipeWrite, Buffer^, READ_BUFFER_SIZE, BytesRead, nil);
            if BytesRead > 0 then
            begin
        ...
        //ShowMessage(output);                                                  //Decide what to do with output here. Log It?
        //WaitForSingleObject(ProcInf.hProcess, INFINITE);
      finally
        //CloseHandle(ProcInf.hThread);
...
begin
  if show then
    hProcess := ProgramStarter(prog, workdir, params, SW_SHOWDEFAULT, output,  bSilent)
  else
    hProcess := ProgramStarter(prog, workdir, params, SW_HIDE, output, bSilent);
...
var
  ...
  j       : Cardinal;
begin
  hProcess := ProgramStarter(prog, workdir, params, mode, output { use SW_SHOWMINNOACTIVE when more feed back is desired });
  if hProcess=0 then
  begin
...
var
  ...
  j       : Cardinal;
begin
  hProcess := ProgramStarter(prog, workdir, params, SW_SHOWMINNOACTIVE, output);
  if hProcess=0 then
  begin
    result := -1; // The return value of this function is the exit code of the process.
    exit;         // Because exit codes are stored as an unsigned integer, we use -1 to
...
begin
  ...
  begin
    hProcess[i] := ProgramStarter(prog, workdir, params[i], SW_SHOWMINNOACTIVE, output);
    if sleeptime > 0 then
      sleep(sleeptime);
